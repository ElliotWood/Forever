...
- **`RequireDamageDealt` defaults true.** A listener that fires on a dodge, a parry, a miss or a block clears it, and the outcome itself is always the caller's — no `ProcTypeMask` has a bit for one.
