...
```
                 tools/database/wowsims.db (SQLite, gitignored, `make db`)
                                   │
   tools/database/buff_manifest.go │   Spell/SpellEffect/SpellMisc/SpellDuration/SpellAuraOptions
...
(checked in)        sim/core/debuffs_auto_gen.go  buffs_debuffs_auto_gen.ts (gitignored)
     │                     │                              │
...
## Manifest (`tools/database/buff_manifest.go`, package `database`)
...
1. Manifest row: `{Field:"battle_shout", Number:28, Scope:ScopeParty, Proto:ProtoBool (Imp BS not in tree), Kind:KindStatFlat, Go:"BattleShout", Name:"Battle Shout", Owner:Warrior, Talent:nil (Booming Voice is radius-only in Forever), Category:"BattleShoutCategory", SingleAura:true, Stats:[AP,RAP]}`. Rows `bs_solarian_sapphire` (item 30446 absent from `Item`) and `snapshot_bs_*` → `KindAbsent` / `KindFlag`, proto fields kept, no UI input.
...
**Absent in client** (KindAbsent shell, proto field kept, listed in issue): Sanctuary, Sanctity (no SLA), Tranquil Air cast, Wrath of Air, Totem of Wrath, Drums, four neck items, Draenei presence, Misery, Ferocious Inspiration, Shadow Embrace, Screech, Unleashed Rage, Blood Frenzy, Expose Weakness, ISB debuff 17800.

**Caster-only auras** (270/271: Shadow Weaving, Improved Scorch, Hemorrhage) grant the simmed player nothing as external debuffs → KindAbsent with reason; field kept.
