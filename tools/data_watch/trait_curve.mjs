...
for (const name of wanted) {
	...
	const spellIds = new Set(spellNames.filter(s => (s.Name_lang ?? '').toLowerCase() === lower).map(s => s.ID));
	const defs = definitions.filter(d => spellIds.has(d.SpellID) || (d.OverrideName_lang ?? '').toLowerCase() === lower);
	...
	for (const point of points) {
		const curve = (byCurve.get(point.CurveID) ?? []).sort((a, b) => Number(a.OrderIndex) - Number(b.OrderIndex)).map(c => `${c.Pos_0}:${c.Pos_1}`);
