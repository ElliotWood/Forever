...
var rankEnumTypes = []struct {
	...
	{"SpellEffectType", "SpellDataEffectKind", "EffectType", "What the effect does - E_SCHOOL_DAMAGE, E_APPLY_AURA."},
	{"EffectAuraType", "SpellDataAura", "AuraType", "Which aura it applies, when the effect is E_APPLY_AURA."},
}
...
// The store's rows name the same two enums, in its own package and its own types. A value dbc has
// no name for keeps its number, marked so a reader is not left looking for a constant.
...
// The names the store's rows reference, as the types sim/core/spelldata declares for them. A second
// namer renders this file, so the values the store uses cannot pull constants into the shared
// package's file, which only the class tables read.
func (n *rankEnumNamer) renderStoreEnums() ([]byte, error) {
	var b strings.Builder
	b.WriteString("// Code generated by tools/database/gen_spelldata. DO NOT EDIT.\n")
	b.WriteString("//\n")
	b.WriteString("// The aura and effect names the store's rows reference, mirrored from\n")
	b.WriteString("// tools/database/dbc/enums.go. Only the values the rows use are emitted, and both sides come\n")
	...
	for _, t := range rankEnumTypes {
		...
		for _, v := range values {
			fmt.Fprintf(&b, "\t%s %s = %d\n", n.byType[t.dbcType][v], t.storeType, v)
		}
		fmt.Fprintf(&b, ")\n")
	}
