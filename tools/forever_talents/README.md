...
> **On the `official` branch:** wowsims/forever generates `ui/sim/talents/trees` from the client
> (`tools/database`), in a leaner schema (one `spellId`, no tooltip text). Don't run
> `import_talents.py` or `apply_beta_tooltips.py` here: they write the Classic-era layout over the
> generated trees. `data/`, `overrides/` and `diff_trees.py` stay useful as the pre-beta reference
> and for checking a client export against it.

Talent trees for all nine classes, used to generate `ui/sim/talents/trees/*.json` and the
...
1. Export the trees from the beta client in the `data/<class>.json` schema, one file per
   class, into a directory. A tree json in the `ui/sim/talents/trees` layout works too,
