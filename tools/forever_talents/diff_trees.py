...
#
# The sim's side is ui/sim/talents/trees/<class>.json for everything structural (which
	...
	for tree_idx, tree in enumerate(trees):
		...
		for talent in sorted(tree['talents'], key=lambda t: (t['row'], t['col'])):
			...
			records.append({
				...
				'name': talent.get('name') or talent.get('fancyName'),
				'tree': tree['name'],
				'treeIdx': tree_idx,
				'row': talent['row'],
				'col': talent['col'],
				'maxRank': talent['maxRank'],
				'spellIds': talent.get('spellIds') or ([talent['spellId']] if 'spellId' in talent else None),
...
def main():
	...
	parser.add_argument('--tree-schema', action='store_true', help='the dataset is in the ui/sim/talents/trees layout')
