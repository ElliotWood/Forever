...
def export_class(client, class_name):
	...
	sim_names = {norm(t.get('name') or t['fancyName']) for tree in sim_trees for t in tree['talents']}
	...
	for tab in range(3):
		...
		sim = max(sim_trees, key=lambda tr: len(keys & {norm(t.get('name') or t['fancyName']) for t in tr['talents']}))
