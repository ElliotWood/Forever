...
// does), and every image the UI source references: literal wow.zamimg.com image URLs,
// paths under WOWHEAD_IMAGES and the bare icon file names of talent tree icon tables.
//
...
var (
	...
	treesDir   = flag.String("trees", "ui/sim/talents/trees", "talent tree directory")
...
var (
	...
	// forever-next's UI (upstream's layout) still names the Wowhead URL itself.
	zamimgUrlRegex = regexp.MustCompile(`zamimg\.com/images/(?:wow/)?([A-Za-z0-9_./\-]+\.(?:jpg|png|gif))`)
...
func main() {
	...
	for _, treeFile := range trees {
		...
		for _, tree := range classTrees {
			...
			for _, talent := range tree.Talents {
				...
				for _, spellId := range append(talent.SpellIds, talent.SpellId) {
	...
	filepath.WalkDir(*uiDir, func(path string, d os.DirEntry, err error) error {
		...
		case ".ts", ".tsx":
			...
			for _, match := range zamimgUrlRegex.FindAllStringSubmatch(source, -1) {
				images[match[1]] = true
			}
