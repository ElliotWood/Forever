...
// OnDamage is called after damage is calculated. Use it for proc effects
// or anything that comes from the final result of dealing damage.
...
func (at *auraTracker) registerAura(unit *Unit, aura Aura) *Aura {
	...
	*newAura = aura
	...
	newAura.onRageChangeIndex = Inactive
...
func (unit *Unit) GetOrRegisterAura(aura Aura) *Aura {
	...
	if curAura == nil {
		...
		curAura.OnRageChange = aura.OnRageChange
		return curAura
	}
}
...
func (at *auraTracker) reset(sim *Simulation) {
	...
	at.onRageChangeAuras = at.onRageChangeAuras[:0]
