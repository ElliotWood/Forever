...
// can fire it, auto or special, and no other hit can. A main-hand auto that
...
func TestWindfuryTotemProcsOffMainHandHitsWithTheChargesTheTriggerLeaves(t *testing.T) {
	setup := func(t *testing.T) (*core.Simulation, *core.Unit, *core.Aura, *[]int32) {
		...
		player := &proto.Player{
			...
			Spec:          DefaultOptions,
			Rotation:      core.GetAplRotation("../../../ui/specs/warrior/dps/apls", "fury").Rotation,
		}
	...
	offer := func(sim *core.Simulation, target *core.Unit, trigger *core.Aura, started *[]int32, procMask core.ProcMask) bool {
	...
	for _, row := range []struct {
		...
		t.Run(row.name, func(t *testing.T) {
			...
			if !offer(sim, target, trigger, started, row.procMask) {
