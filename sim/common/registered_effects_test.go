...
// Which items and enchants the sim simulates an effect for, as a checked-in list. The generated
// registrations are a hundred lines of ids; this file is the review surface for the only question a
// regeneration has to answer - what became live, and what stopped being live.
//
...
func TestRegisteredEffects(t *testing.T) {
	...
	if os.Getenv("UPDATE_REGISTERED_EFFECTS") != "" {
	...
	t.Errorf("%s is not what the sim registers; regenerate it in the commit that changed the registrations:\n"+
